...
# Usage: bash az.sh <command> [options]
set -Eeuo pipefail
...
usage() {
  cat <<'USAGE'
Alazab bot operations helper (Unified CLI).
...
Commands:
  ...
  db-init             Install UberFix DB schema (requires UBERFIX_DB_URL).
  db-sync             Sync Supabase Cloud migrations and functions.
  wsl-setup           Setup WSL environment for the bot.
  wsl-fix             Fix WSL networking/port issues.

  # --- Environment ---
  on                  Enter a subshell with the Python virtual environment activated.
  off                 Exit the virtual environment subshell.


  # --- Code & Data Management ---
  sync                Sync repository changes using git-sync.
  clean               Deep clean pycache, runtime files, and temp logs.
  fix-yaml            Fix Rasa YAML syntax and formatting.
  check               Run general static checks.
  audit               Run backend security & code audit.
  audit-fix           Auto-fix backend audit issues.

  # --- Integration ---
  register-meta       Register Facebook/Meta Webhooks.
  register-tg         Register Telegram Webhooks.
  ssl                 Setup SSL certificates (Local/Prod).
...
activate_venv() {
  choose_python
  [[ -d "$VENV_DIR" ]] || fail "Virtualenv not found: $VENV_DIR. Run: bash az.sh setup"
...
cmd_setup() {
  ...
  if [[ -f pyproject.toml ]]; then
    run_checked ~/.local/bin/uv pip install -e ".[dev]"
  else
    run_checked ~/.local/bin/uv pip install -r requirements.txt
  fi
  run_checked ~/.local/bin/uv pip install supabase
...
cmd_validate() {
  ...
  run_checked rasa data validate --domain "$RUNTIME_DOMAIN_FILE" --endpoints endpoints.yml
}
...
cmd_preflight() {
  cmd_doctor
  load_env
  require_env RASA_PRO_LICENSE OPENAI_API_KEY DB_NAME DB_USER DB_PASSWORD
  cmd_validate
}
...
cmd_start() {
  ...
  for port in "$ACTIONS_PORT" "$RASA_PORT" "$WEBHOOK_PORT"; do
    if port_busy "$port"; then
      fail "Port $port is already in use. Run: bash az.sh start --force"
    fi
  done
...
cmd_dev() {
  ...
  ok "══════════════════════════════════════════"
  ok " لوقف كل شيء: bash az.sh stop"
  ok " للسجلات:     bash az.sh logs all"
  ok "══════════════════════════════════════════"
}
...
cmd_db_init() {
  bash scripts/install-uberfix-db.sh "$@"
}
...
cmd_clean() {
  choose_python
  "$PYTHON_BIN" scripts/deep_clean.py "$@"
}

cmd_fix_yaml() {
  choose_python
  "$PYTHON_BIN" scripts/fix_yaml_syntax.py "$@"
}

cmd_audit() {
  bash scripts/backend-audit.sh "$@"
}

cmd_audit_fix() {
  choose_python
  "$PYTHON_BIN" scripts/backend_audit_fix.py "$@"
}
...
cmd_on() {
  ...
  choose_python
  [[ -d "$VENV_DIR" ]] || fail "Virtualenv not found: $VENV_DIR. Run: bash az.sh setup"
  log "Entering virtual environment... (Type 'exit' or 'bash az.sh off' to leave)"
  bash --rcfile <(cat ~/.bashrc 2>/dev/null; echo "source '$ROOT_DIR/$VENV_DIR/bin/activate'") -i
}

cmd_off() {
  if [[ -n "${VIRTUAL_ENV:-}" ]]; then
    log "Exiting virtual environment..."
    kill -9 $$
  else
...
case "$cmd" in
  ...
  db-sync) cmd_db_sync "$@" ;;
  sync) cmd_sync "$@" ;;
  register-meta) cmd_register_meta "$@" ;;
  register-tg) cmd_register_tg "$@" ;;
  clean) cmd_clean "$@" ;;
  fix-yaml) cmd_fix_yaml "$@" ;;
  audit) cmd_audit "$@" ;;
  audit-fix) cmd_audit_fix "$@" ;;
  check) cmd_check "$@" ;;

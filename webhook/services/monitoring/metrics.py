"""webhook/services/monitoring/metrics.py — عدادات أداء في الذاكرة"""
...
def inc_channel(channel: str) -> None:
    with _lock: _channel_counts[channel] += 1
...
def inc_endpoint(path: str, latency_ms: float, is_error: bool = False) -> None:
    with _lock:
        ...
        if is_error: _error_counts[path] += 1
...
def snapshot() -> dict:
    with _lock:
        ...
        for path, count in _endpoint_counts.items():
            ...
            endpoint_stats[path] = {"count": count, "errors": _error_counts.get(path,0),
                "p50_ms": _percentile(lats,50), "p95_ms": _percentile(lats,95), "p99_ms": _percentile(lats,99)}
        return {"uptime_seconds": round(time.time()-_start), "channels": dict(_channel_counts), "endpoints": endpoint_stats}

def _percentile(data: list[float], pct: int) -> float | None:
    if not data: return None
    s = sorted(data); idx = int(len(s)*pct/100)

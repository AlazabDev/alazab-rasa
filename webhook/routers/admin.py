...
def _login_rate_check(ip: str) -> bool:
    """True إذا مسموح بالمحاولة، False إذا تجاوز الحد (10 محاولات / 5 دقائق)."""
    try:
        import redis as _r, hashlib
        r = _r.Redis(
        ...
        return int(r.get(key) or 0) < 10
    except Exception:
        return True  # Redis غير متاح — اسمح بالمحاولة بدل حظر الجميع
...
def _login_rate_record(ip: str) -> None:
    try:
        ...
        key = f"login_fail:{hashlib.md5(ip.encode(), usedforsecurity=False).hexdigest()[:12]}"
        pipe = r.pipeline(); pipe.incr(key); pipe.expire(key, 300); pipe.execute()
    except Exception:
        pass

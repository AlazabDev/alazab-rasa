...
import hashlib, logging, os, secrets, time, uuid
...
_RATE_LIMITS: dict[str, tuple[int,int]] = {
    "/chat/tts": (10,60), "/chat/audio": (10,60), "/chat/upload": (20,60),
    "/chat": (30,60), "/admin/login": (10,300),
}

def _redis_client():
    try:
        import redis
        return redis.Redis(host=os.getenv("REDIS_HOST","127.0.0.1"), port=int(os.getenv("REDIS_PORT","6379")),
...
def _rate_check(ip: str, path: str) -> tuple[bool,int,int]:
    prefix = next((p for p in sorted(_RATE_LIMITS, key=len, reverse=True) if path.startswith(p)), None)
    if not prefix: return True, 999, 0
    max_req, window = _RATE_LIMITS[prefix]
    r = _redis_client()
    if r is None: return True, max_req, 0
    ip_hash = hashlib.md5(ip.encode(), usedforsecurity=False).hexdigest()[:12]
    bucket = int(time.time()) // window
    key = f"rl:{ip_hash}:{prefix.replace('/','_')}:{bucket}"
    try:
        pipe = r.pipeline(); pipe.incr(key); pipe.expire(key, window*2)
        count = pipe.execute()[0]
    except Exception as exc:
        logger.warning("Rate-limit Redis error: %s", exc); return True, max_req, 0
    if count > max_req:
        return False, 0, window - (int(time.time()) % window)
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not allowed:
            ...
            return JSONResponse(status_code=429, content={"detail":"طلبات كثيرة — انتظر قليلاً"},
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        h["Content-Security-Policy"] = (
            f"default-src 'self'; script-src 'self' 'nonce-{nonce}' https://fonts.googleapis.com; "
            f"style-src 'self' 'nonce-{nonce}' https://fonts.googleapis.com; "
            f"font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; "
            f"connect-src 'self' https://api.openai.com https://*.supabase.co; frame-ancestors 'none';"
        )
        return response
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if path not in self._SKIP:
            ...
            self._log.log(level, "%s %s → %s | %.1fms | ip=%s | req=%s",
                request.method, path, response.status_code, latency_ms, _get_client_ip(request), req_id)

"""
webhook/auth.py — AzaBot Auth v4.1 (إصدار الإنتاج)
====================================================

الإصلاحات:
  ✅ bcrypt بدل plain-text password comparison
  ✅ كلمة مرور مستقلة لكل مستخدم (ADMIN_PASSWORD_HASH_*)
  ✅ جلسات Redis بدل sessions.json (مقاوم لـ restart + multi-process)
  ✅ Timing-safe user enumeration prevention
  ✅ Session TTL قابل للضبط
"""
...
# ══════════════════════════════════════════════════════════════
#  المستخدمون — كلٌّ له متغير بيئة مستقل
# ══════════════════════════════════════════════════════════════
...
if not _SESSION_SECRET:
    # في dev نسمح بقيمة افتراضية مع تحذير
    import sys
    if os.getenv("NODE_ENV") == "production":
        raise RuntimeError("[FATAL] ADMIN_SESSION_SECRET غير مضبوط — أوقف التشغيل")
    _SESSION_SECRET = "dev-secret-not-for-production-use"
    print("[WARN] auth.py: ADMIN_SESSION_SECRET غير مضبوط — وضع dev فقط", file=sys.stderr)
...
def _get_user_hash(email: str) -> Optional[bytes]:
    ...
    if not stored:
        logger.warning("لم يُضبَط %s في .env — المستخدم %s معطَّل", user["_env_key"], email)
        return None
...
def verify_password(plain: str, email: str) -> bool:
    ...
    if not stored:
        bcrypt.checkpw(b"dummy", _DUMMY_HASH)  # timing guard
        return False
    try:

...
ALLOWED_ORIGINS = [origin.strip().rstrip("/") for origin in ALLOWED_ORIGINS.split(",") if origin.strip()]
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers["Content-Security-Policy"] = (
            "default-src 'self'; "
            "script-src 'self' 'unsafe-inline' https://bot.alazab.com; "
            "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; "
            "font-src 'self' https://fonts.gstatic.com; "
            "img-src 'self' data: https:; "
            "connect-src 'self' https://bot.alazab.com https://api.openai.com;"
        )
        return response
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "details": exc.errors(),
            "message": "يرجى التأكد من صحة البيانات المرسلة"
        },
    )
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "message": "نعتذر، واجهنا مشكلة تقنية. تم تسجيل الخطأ للمراجعة."
        },
    )
...
async def _record_conversation(
    ...
    *,
    ...
    if attachment:
        safe_attachment = _serialize_attachment(attachment)
...
def _uberfix_transition_stage(
    ...
    _insert_request_note(cur, row["id"], f"تغيير المرحلة من {from_stage} إلى {to_stage}: {reason}", "system", "bot")
    _insert_uberfix_audit(
        cur,
        ...
        row,
        updated,
        {"from_stage": from_stage, "to_stage": to_stage, "reason": reason, "session_id": session_id},
    )
...
def _maintenance_request_public(row: dict[str, Any]) -> dict[str, Any]:
    return _jsonable({
        ...
        "track_url": row.get("track_url") or f"{UBERFIX_TRACK_BASE_URL}/{row.get('request_number') or row.get('id')}",
...
async def health():
    """فحص صحة Rasa والقنوات المفعلة وقاعدة البيانات."""
    rasa_ok = False
    try:
    ...
    def _check_db():
        try:
            with _uberfix_db_connect() as conn:
                with conn.cursor() as cur:
    ...
    # معلومات إضافية للمهنيين
    import psutil
    process = psutil.Process(os.getpid())
    memory_info = process.memory_info()

    return {
        "status": "ok" if (rasa_ok and db_ok) else "degraded",
...
@app.get("/admin/uploads/{upload_id}/download", tags=["System"])
async def admin_download_upload(upload_id: str, _: None = Depends(_require_admin)):
    data = _load_admin_data()
    upload = next((item for item in data.get("uploads", []) if item.get("id") == upload_id), None)
    if not upload:
        raise HTTPException(status_code=404, detail="Upload not found")

    file_path = Path(str(upload.get("path") or "")).expanduser().resolve()
    if not _is_relative_to(file_path, UPLOADS_ROOT):
        raise HTTPException(status_code=403, detail="Upload path is outside the uploads directory")
    if not file_path.is_file():
        raise HTTPException(status_code=404, detail="Upload file missing on disk")

    return FileResponse(
        file_path,
        filename=str(upload.get("name") or file_path.name),
        media_type=str(upload.get("content_type") or "application/octet-stream"),
    )
...
async def admin_api(request: Request, action: str, _: None = Depends(_require_admin)):
    ...
    if action == "list_uploads":
        ...
        return [_serialize_attachment(item) for item in uploads[:500]]
    ...
    if action == "get_conversation":
        ...
        return {**conv, "messages": _serialize_conversation_messages(conv.get("messages", []))}
...
async def meta_verify(
    ...
    if not META_VERIFY:
        raise HTTPException(status_code=503, detail="Meta verification token is not configured")
...
async def telegram_messages(request: Request, background_tasks: BackgroundTasks):
    ...
    يُسجَّل عبر:
      POST https://api.telegram.org/bot{TOKEN}/setWebhook
        ?url=https://bot.alazab.com/webhook/telegram&secret_token={TELEGRAM_WEBHOOK_SECRET}
    """
    if not TG_WEBHOOK_SECRET:
        raise HTTPException(status_code=503, detail="Telegram webhook secret is not configured")
    if not hmac.compare_digest(request.headers.get("X-Telegram-Bot-Api-Secret-Token", ""), TG_WEBHOOK_SECRET):
...
async def _notify_all_channels(lead: LeadData) -> None:
    ...
    # CRM Webhook
    if WEBHOOK_NOTIFY:
        if _is_internal_lead_notify_url(WEBHOOK_NOTIFY):
            logger.warning(
...
def _is_internal_lead_notify_url(value: str) -> bool:
    try:
...
def _build_file_prompt(
    ...
    attachment_reference = attachment.get("url") or attachment.get("path") or attachment.get("relative_path") or attachment["name"]
    return (
        ...
        f"مرجع الملف: {attachment_reference}\n"
        f"{user_note}"
...
def _build_audio_prompt(
    ...
    return (
        ...
        f"البراند: {brand or 'غير محدد'}\n"
        f"الموقع: {site_host or 'غير محدد'}\n"
        f"مرجع التسجيل: {attachment_reference}\n"
...
def _serialize_conversation_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for message in messages:
        ...
        if isinstance(attachments, list):
            item["attachments"] = [
                _serialize_attachment(attachment) if isinstance(attachment, dict) else attachment

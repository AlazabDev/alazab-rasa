...
=================================================
✅ lifespan (بدل on_event المُهجَر) + httpx client موحّد
✅ /docs مغلق في الإنتاج | /health عامة + /health/details للأدمن
...
from .config import (
    ...
    FRONTEND_EMBED_DIR, META_TOKEN, RASA_URL, STATIC_DIR, TG_TOKEN, WA_TOKEN,
)
...
_LOG_FMT = (
    ...
    if _IS_PROD else "%(asctime)s [%(levelname)s] %(name)s: %(message)s"
)
...
async def health_details(request: Request):
    """فحص تفصيلي لكل الخدمات + metrics — للأدمن فقط."""

...
def generate_nlu_data():
    ...
    for item in sample_items:
        intent_examples["ask_alazab_technical_specs"].append(
            f"- ماهي مواصفات [ {item} ](service_item)؟"
        )
        intent_examples["ask_alazab_technical_specs"].append(
            f"- محتاج اعرف تفاصيل عن [ {item} ](service_item)"
        )

        intent_examples["ask_uberfix_request"].append(
            f"- محتاج [ صيانة ](action_type) لـ [ {item} ](service_item)"
        )
        intent_examples["ask_uberfix_request"].append(
            f"- عندي مشكلة في [ {item} ](service_item)"
        )

        intent_examples["ask_alazab_quote"].append(
            f"- سعر [ توريد وتركيب ](action_type) [ {item} ](service_item) كام؟"
        )
        intent_examples["ask_alazab_quote"].append(
            f"- محتاج عرض سعر لـ [ {item} ](service_item)"
        )

    for intent, examples in intent_examples.items():
        nlu_content["nlu"].append({"intent": intent, "examples": "\n".join(examples)})

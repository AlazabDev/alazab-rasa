...
│   │   ├── docker-compose.prod.yaml    # Stack الإنتاج
│   │   ├── Dockerfile                  # صورة webhook البسيطة
│   │   ├── Dockerfile.webhook          # FastAPI webhook server
│   │   ├── Dockerfile.actions          # Rasa actions server
│   │   └── azabot/docker-compose.yml   # Stack الواجهة الثابتة/SSL
...
│
├── 🎨 Chatbot Widget
│   └── webhook/static/widget/       # سكربت التضمين وصفحة المعاينة
│
├── 🧪 Tests
...
### تشغيل الواجهة والباك اند معاً من جذر المشروع

على Windows PowerShell:

```powershell
cd D:\A\chatbot\alazab-rasa
.\start-dev.ps1 -SkipTrain
```
...
إذا كان الباك اند يعمل بالفعل وتريد تشغيل الواجهة فقط:

```powershell
.\start-frontend.ps1 -SkipInstall
```

على WSL/Ubuntu:

```bash
cd /mnt/d/A/chatbot/alazab-rasa
SKIP_TRAIN=true bash start-dev.sh
```
...
make prod-readiness # فحص جاهزية ملفات الإنتاج
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\test-preflight.ps1 -EnvFile .env -ComposeFile docker/docker-compose.yaml
```

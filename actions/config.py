...
# ✅ بدون project ID مكتوب صريحاً — يجب ضبط BOT_GATEWAY_URL في .env
BOT_GATEWAY_URL = os.getenv("BOT_GATEWAY_URL", "").rstrip("/")

BOT_API_KEY = os.getenv("BOT_API_KEY", os.getenv("UBERFIX_API_KEY", "")).strip()

# maintenance-gateway (لتغيير المراحل من الخارج فقط)
# ✅ بدون project ID مكتوب صريحاً — يجب ضبط MAINTENANCE_GATEWAY_URL في .env
MAINTENANCE_GATEWAY_URL = os.getenv("MAINTENANCE_GATEWAY_URL", "").rstrip("/")

# ── Startup Validation ────────────────────────────────────────
def _validate() -> None:
    import logging
    log = logging.getLogger("alazab.actions.config")
    for name, val in (("BOT_GATEWAY_URL", BOT_GATEWAY_URL), ("MAINTENANCE_GATEWAY_URL", MAINTENANCE_GATEWAY_URL)):
        if not val:
            log.warning("[actions.config] ⚠️  %s غير مضبوط — تكامل UberFix لن يعمل", name)

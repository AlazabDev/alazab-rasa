...
class ActionCorrectFlowSlot(Action):
    """Safe handler for Rasa CALM slot-correction pattern.

    Rasa Pro can trigger `pattern_correction` when the user corrects a
    previously collected value. This action does not guess the new value; it
    records the user's correction text for downstream actions and lets CALM
    ...
    def run(
        ...
        correction_text = (tracker.latest_message.get("text") or "").strip()
        events: List[Dict[Text, Any]] = []

        if correction_text:
            events.append(SlotSet("last_correction_text", correction_text))
            dispatcher.utter_message(
                text="تم استلام التصحيح. سأكمل بناءً على آخر معلومة ذكرتها."
            )
        else:
            dispatcher.utter_message(
                text="اكتب المعلومة الصحيحة وسأحدثها في سياق الطلب."
            )

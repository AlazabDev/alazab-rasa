      ...
      for (const entry of entries){
        ...
        for (const change of changes){
          ...
          for (const message of messages){
            ...
            let projectId = null;
            if (projectNumber) {
              const { data: project } = await supabase.from("projects").select("id").eq("project_number", projectNumber).single();
              projectId = project?.id || null;
            }
            // If no project found, assign to a default/unassigned project
            if (!projectId) {
              const { data: defaultProject } = await supabase.from("projects").select("id").eq("project_number", "UNASSIGNED").single();
              if (defaultProject) {
            ...
            const mediaUrlRes = await fetch(`https://graph.facebook.com/${apiVersion}/${mediaId}`, {
              headers: {
            ...
            const { error: uploadError } = await supabase.storage.from("project-files").upload(storagePath, mediaBlob, {

...
// ========== AWS S3 ==========
async function s3Request(method, path, config, body, contentType) {
  ...
  const hmac = async (key, msg)=>{
    const cryptoKey = await crypto.subtle.importKey("raw", key instanceof ArrayBuffer ? key : key, {
      name: "HMAC",
      hash: "SHA-256"
    }, false, [
      "sign"
  ...
  const signature = Array.from(new Uint8Array(await crypto.subtle.sign("HMAC", await crypto.subtle.importKey("raw", signingKey, {
    name: "HMAC",
    hash: "SHA-256"
  }, false, [
    "sign"
  ]), encoder.encode(stringToSign)))).map((b)=>b.toString(16).padStart(2, "0")).join("");
...
async function getS3DownloadUrl(config, key) {
  const { config: creds, bucket_name, region } = config;
  const reg = region || creds.region || "us-east-1";
  return `https://${bucket_name}.s3.${reg}.amazonaws.com/${key}`;
}
// ========== OCI Object Storage ==========
async function ociRequest(method, url, _config) {
...
async function listOciObjects(config, prefix = "") {
...
// ========== GCP Cloud Storage ==========
async function listGcpObjects(config, prefix = "") {
  const { config: creds, bucket_name } = config;
  ...
  try {
    ...
    if (action === "delete-object") {
      ...
      if (provider.provider_type === "s3") {
        await deleteS3Object(provider, key);
      }
      return new Response(JSON.stringify({
        success: true
      }), {
    ...
    if (action === "get-download-url") {
      ...
      let url = "";
      if (provider.provider_type === "s3") {
        url = await getS3DownloadUrl(provider, key);
      }
      return new Response(JSON.stringify({
        url
      }), {

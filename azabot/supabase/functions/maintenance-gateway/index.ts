...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const category = body.fault_category && VALID_CATEGORIES.includes(body.fault_category) ? body.fault_category : "other";
    ...
    if (body.source_reference) {
      const { data: existing } = await supabase.from("maintenance_requests").select("id, ticket_number").eq("source_reference", body.source_reference).eq("source", body.source).maybeSingle();
      if (existing) {
        return new Response(JSON.stringify({
          status: "duplicate",

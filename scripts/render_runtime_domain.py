...
def main() -> int:
    ...
    if ENDPOINTS_SRC.exists():
        import os
        text = ENDPOINTS_SRC.read_text(encoding="utf-8")
        redis_pw = os.environ.get("REDIS_PASSWORD", "").strip()
        if not redis_pw or redis_pw == "REPLACE_ME":
            text = text.replace('  password: "${REDIS_PASSWORD}"\n', "")
        ENDPOINTS_OUT.write_text(text, encoding="utf-8")

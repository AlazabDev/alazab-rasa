...
load_env() {
  [[ -f "$ENV_FILE_PATH" ]] || fail "Missing $ENV_FILE_PATH. Add real production values before deploy."
...
validate_env() {
  ...
  if [[ "${DB_HOST:-}" == "postgres" || "${REDIS_HOST:-}" == "redis" ]]; then
    fail "DB_HOST/REDIS_HOST still use Docker service names. Set real production hosts in $ENV_FILE_PATH."
  fi

  if [[ "${UPLOADS_PUBLIC_ENABLED:-false}" == "true" ]]; then
    warn "UPLOADS_PUBLIC_ENABLED=true will expose uploaded files publicly. Prefer false in production."
  fi

  [[ -f "$ENDPOINTS_FILE_PATH" ]] || fail "Missing endpoints file: $ENDPOINTS_FILE_PATH"
}
...
install_database_schema() {
  ...
  local args=(--env-file "$ENV_FILE_PATH")
...
write_systemd_services() {
  ...
  local common_env="EnvironmentFile=${ENV_FILE_PATH}
...
ExecStart=${APP_DIR}/.venv/bin/rasa run --enable-api --endpoints ${ENDPOINTS_FILE_PATH} --cors \${ALLOWED_ORIGINS} --port 5005
...
main() {
  ...
  ENDPOINTS_FILE_PATH="$(resolve_path "$ENDPOINTS_FILE")"
  load_env
  validate_env
  ...
  if [[ -n "${UPLOADS_DIR:-}" ]]; then
    mkdir -p "$(dirname "$UPLOADS_DIR")" "$UPLOADS_DIR"
  fi
  if [[ -n "${ADMIN_DATA_FILE:-}" ]]; then
    mkdir -p "$(dirname "$ADMIN_DATA_FILE")"
  fi
  install_python_deps
  install_database_schema
  install_frontend

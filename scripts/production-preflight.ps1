param(
    [string]$EnvFile = ".env",
    [string]$ComposeFile = "docker/docker-compose.prod.yaml"
)
...
if (Test-Path $EnvFile) {
    ...
    if ($envMap.ContainsKey("UPLOADS_PUBLIC_ENABLED") -and $envMap["UPLOADS_PUBLIC_ENABLED"].ToLower() -eq "true") {
        Warn "UPLOADS_PUBLIC_ENABLED=true will expose uploaded files publicly."
    } else {
        Pass "UPLOADS_PUBLIC_ENABLED is private or unset"
    }

    $origins = @($envMap["ALLOWED_ORIGINS"] -split ",") | ForEach-Object { $_.Trim() } | Where-Object { $_ }
    $publicBaseUrl = $envMap["PUBLIC_BASE_URL"].TrimEnd("/")
    if ($origins -contains $publicBaseUrl) {
        Pass "CORS includes PUBLIC_BASE_URL"
    } else {
        Fail "CORS missing PUBLIC_BASE_URL ($publicBaseUrl)"
    }
}

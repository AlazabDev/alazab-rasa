...
def yaml_files() -> list[Path]:
    return (
        [
            ...
            ROOT / "config.yml",
            ROOT / "endpoints.yml",
            ROOT / "endpoints.sqlite.yml",
        ]
        + sorted((ROOT / "domain").glob("*.yml"))
        + sorted((ROOT / "data").rglob("*.yml"))
    )
    ...
    if isinstance(intents, list):
        ...
        for item in intents:
            key = (
                ...
                else next(iter(item.keys()), str(item))
        ...
        if len(cleaned) != len(intents):
            ...
            say(
                "ok",
                f"removed {len(intents)-len(cleaned)} duplicate domain intent mappings",
            )
        else:

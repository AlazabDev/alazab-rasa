...
آخر تحديث: 2026-05-11 (إصدار الإنتاج المتكامل)

> **تحديث الإنتاج V2.0**:
> - تم إضافة الربط المحاسبي الكامل مع **نظام دفترة (Daftra)** للمزامنة وإصدار الفواتير.
> - تم تفعيل **بوصلة آل عزب (Brand Navigator)** للتوجيه الذكي بين مواقع المجموعة.
> - تم توحيد وهيكلة بيانات التدريب (NLU/Stories/Rules) لضمان أعلى دقة.
...
### E. المحاسبة والتوجيه (Daftra & Navigator)

#### 1️⃣5️⃣ `daftra_sync_client` — مزامنة العميل مع دفترة
يتم استدعاؤه آلياً بعد طلب الصيانة لضمان وجود سجل مالي للعميل.

#### 1️⃣6️⃣ `daftra_create_invoice` — إصدار فاتورة إلكترونية
إنشاء فاتورة "مسودة" أو "نهائية" في نظام دفترة مرتبطة بطلب الصيانة الحالي.

#### 1️⃣7️⃣ `brand_navigator` — بوصلة آل عزب
توجيه العميل لأي من مواقع المجموعة بناءً على اهتمامه (معماري، تشطيب، هوية، صيانة، توريدات).
